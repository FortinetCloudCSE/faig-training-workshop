...
scripts/faig/                  — lab automation students actually run
  ...
  plans/, specs/               — design docs for the chart (tracked; /plans/ ignore is root-only)
...
- **Plan/log/spec files go in root-level `plans/`, and in THIS repo they stay untracked.** `.gitignore` lines 10–11 are `/plans/` and `/specs/` — **root-anchored on purpose**. Write plan files to `plans/` as normal; they live on your machine only. Do not `git add -f` them. Root-level `plans/` is still the right directory rather than `docs/plans/`: it is inert to Hugo (which reads only `content/`, `layouts/`, `static/`, `assets/`, `data/`, `i18n/`, `archetypes/`, `themes/`) and outside `FOLDERS_TO_DELETE`, so nothing publishes or gets deleted either way.
  ...
  - **Why the leading slash matters.** Tom Walsh added the entries as bare `plans/` / `specs/` in `884d409` (2026-07-23). Without a leading slash those patterns match at **any** depth, so they also caught `scripts/faig/plans/` and `scripts/faig/specs/` — the chart's own design docs. He committed three more of them the next day (`bb7a615`, `c88c7fc`), which means force-adding past his own rule. Anchoring to `/plans/` and `/specs/` keeps the repo root clean, as intended, while letting `scripts/faig/plans|specs/` be added normally. **Never drop the leading slash.**
